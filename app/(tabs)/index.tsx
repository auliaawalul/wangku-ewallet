...
import { useLanguage } from '../LanguageContext'; // 👈 Mengambil Pemancar Bahasa Pusat

const { width } = Dimensions.get('window');

export default function HomeScreen() {
  const { t } = useLanguage(); // 👈 Ambil kamus terjemahan pusat
  const [displayName, setDisplayName] = useState('User');

  useEffect(() => {
    const auth = getAuth();
    
    // Listener otomatis untuk memantau nama user yang login dari Firebase Auth
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.header}>
        <View>
          <Text style={styles.welcomeText}>{t.welcome}</Text> {/* 👈 Menggunakan Bahasa Global */}
      ...
      <View style={styles.balanceCard}>
        <View style={styles.cardHeader}>
          <Text style={styles.balanceTitle}>{t.balance}</Text> {/* 👈 Menggunakan Bahasa Global */}
          <TouchableOpacity>
...
{/* Grid Menu Utama - 4 Fitur Aktif Tersambung */}
      <View style={styles.menuGrid}>
        ...
        <TouchableOpacity style={styles.menuItem} onPress={() => router.push('/pay')}>
          ...
          <Text style={styles.menuLabel}>{t.payBtn}</Text> {/* 👈 Ganti bagian ini */}
        </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.menuItem} onPress={() => router.push('/modal')}>
          ...
          <Text style={styles.menuLabel}>{t.modalBtn}</Text> {/* 👈 Ganti bagian ini */}
        </TouchableOpacity>
      </View>

      {/* Informasi Transaksi Terbaru */}
      <View style={styles.transactionSection}>
        <Text style={styles.sectionTitle}>{t.historyTitle}</Text> {/* 👈 Menggunakan Bahasa Global */}
