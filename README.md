...
Aplikasi ini memenuhi ketentuan EAS Praktikum Mobile Computing karena menggabungkan:
...
```txt
D:\expo_mobcomp\wangku-ewallet\api\db.json
const API_BASE_URL = "http://192.168.18.53:3001";
```

Contoh API JSON yang berisi produk digital:
```json
"digitalProducts": [
    {
      "id": 1,
      "type": "pulsa",
      "provider": "Telkamsel",
      "name": "Pulsa Telkamsel 10.000",
      "description": "Pulsa reguler Telkamsel nominal Rp10.000",
      "price": 11000,
      "category": "Pulsa"
    },
    {
      ...
      "type": "pulsa",
      "provider": "Indonsat",
      "name": "Pulsa Indonsat 25.000",
      "description": "Pulsa reguler Indosat nominal Rp25.000",
      "price": 26000,
      "category": "Pulsa"
    }
]
...
1. API JSON ini sesuai dengan Fetch Data Axios.
2. Data memiliki id, type, provider, name, price, description, dan category.
3. Dalam aplikasi WangKu, data produk dari JSON API dipakai sebagai daftar produk digital,
...
4. Response API disimpan ke state `digitalProducts`.
5. UI menampilkan produk dalam bentuk list card.
6. User memilih produk digital dan membayar menggunakan saldo WangKu.
...
Fungsi:
1. Mengambil daftar produk digital dari JSON API menggunakan Axios.
...
3. Axios mengambil data dari API JSON.
4. Data API disimpan ke state `digitalProducts`.
5. UI menampilkan daftar produk digital.
...
dan pembayaran produk digital (API). Fitur tambahan yang tersedia adalah top up saldo.

## 15. Kesimpulan
1. Menggunakan Axios untuk HTTP request ke API publik.
2. Menggunakan Firebase Authentication dan Cloud Firestore.
3. Memiliki 3 fitur utama yang berfungsi.
